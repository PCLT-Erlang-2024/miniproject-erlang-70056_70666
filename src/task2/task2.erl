...
allocate_truck(FleetState, BeltNumber, NewTruck) ->
    TruckPid = spawn(task2, truck_operation, [NewTruck]),
...
start_operation(OperationNumber, FleetManagerPID) ->
    ...
    ActiveTruckPID = spawn(task2, truck_operation, [new_truck(OperationNumber, FleetManagerPID)]),
    io:format("[Conveyor Operation] Started Truck Operation ~p~n", [ActiveTruckPID]),
    ActiveBeltPID = spawn(task2, belt_operation, [ActiveTruckPID, OperationNumber]),
    ...
    PackageGeneratorPID = spawn(task2, package_generator, [ActiveBeltPID, OperationNumber]),
...
start() -> 
    ...
    FleetManagerPID = spawn(task2, fleet_manager, [#{trucks => #{}, belts_pids => #{}}]),
